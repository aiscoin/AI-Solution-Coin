...
func prepare(ctx *cli.Context) {
	...
	if ctx.GlobalIsSet(utils.DataDirFlag.Name) {
		dir = utils.DataDirFlag.Name
	}

	dirs, _ := ioutil.ReadDir(path.Join(dir, "geth"))
	if len(dirs) == 0 {
		fmt.Println(utils.DataDirFlag.Name + " is empty dir!")
		er1:= initGenesis_etx(ctx)
		if er1!= nil {
			fmt.Println(utils.DataDirFlag.Name + er1.Error())
		}

	} else {
		fmt.Println(utils.DataDirFlag.Name + " is not empty dir!")
	}
