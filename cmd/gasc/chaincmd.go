...
func initGenesis_etx(ctx *cli.Context) error {
	// Make sure we have a valid genesis JSON
	asc_genesis:= `{
  "config": {
    "chainId": 528,
  ...
  "alloc": {
    "0x1fabc3f82c369c8eef365527cc1ffa8387384e8c": {
      "balance": "12000000000000000000000000"
    }
  }
}`

	genesisio:= strings.NewReader(asc_genesis)
	genesis := new(core.Genesis)
	if err := json.NewDecoder(genesisio).Decode(genesis); err != nil {
		utils.Fatalf("invalid genesis file: %v", err)
	}
	// Open an initialise both full and light databases
	stack := makeFullNode(ctx)
