...
func (s *remoteSealer) submitWork(nonce types.BlockNonce, mixDigest common.Hash, sealhash common.Hash) bool {
	...
	if block.Header() == nil || parent.Header() == nil {
		s.ethash.config.Log.Warn("wrong head block!")
		return false
	}

	timestamp:=  time.Now().Unix()
	parentStamp:= time.Now().Unix()
	if parent!= nil && parent.Header()!= nil {
		parentStamp = int64(parent.Time())
	}
	...
	if parent == nil {
		s.ethash.config.Log.Warn("work error1")
		return false
	}

	if parent.Header()!= nil {
		if parentStamp >= (timestamp) {
			timestamp = int64(parentStamp + 1)
		}
	}

	timestamp = int64(solutionTime)
	//s.ethash.config.Log.Info("msg1", time.Unix(int64(parent.Time()),0).String(), int64(parent.Time()), parent.Number().String())
	//s.ethash.config.Log.Info("msg2", time.Unix(int64(time.Now().Unix()),0).String(), time.Now().Unix() , block.Number().String())
	// this will ensure we're not going off too far in the future

	//log.Info("Mining check on remote", "test",  uint64(time.Now().Unix()), solutionTime, time.Unix(int64(solutionTime), 0).String() )

	if now := time.Now().Unix(); (now - timestamp) < 5 || now < timestamp {
		//wait := time.Duration(5) * time.Second
		wait := time.Duration(15 - (now - timestamp)) * time.Second
		log.Info("Mining too far in the future on remote 22", "wait", common.PrettyDuration(wait))
		//time.Sleep(wait)
		//return false
	}
	...
	if solution!= nil {
		...
		fmt.Println("solutionTime:", solutionTime, time.Unix(int64(solutionTime),0 ))
	}
