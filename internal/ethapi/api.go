...
func (s *PrivateAccountAPI) NewAccount(password string) (common.Address, error) {
	...
	if err == nil {
		log.Info("Your new key was generated", "address", common.AddressToBase58(acc.Address))
...
func (s *PublicBlockChainAPI) GetBalanceV(ctx context.Context, address string, blockNrOrHash rpc.BlockNumberOrHash) (*hexutil.Big, error) {
	state, _, err := s.b.StateAndHeaderByNumberOrHash(ctx, blockNrOrHash)
	if state == nil || err != nil {
		return nil, err
	}
	vAddr:= common.Base58ToAddress2(address)
	return (*hexutil.Big)(state.GetBalance(vAddr)), state.Error()
}
...
// GetCode returns the code stored at the given address in the state for the given block number.
func (s *PublicBlockChainAPI) GetCodeV(ctx context.Context, address string, blockNrOrHash rpc.BlockNumberOrHash) (hexutil.Bytes, error) {
	state, _, err := s.b.StateAndHeaderByNumberOrHash(ctx, blockNrOrHash)
	if state == nil || err != nil {
		return nil, err
	}
	vAddr:= common.Base58ToAddress2(address)
	code := state.GetCode(vAddr)
	return code, state.Error()
}
...
func RPCMarshalHeader(head *types.Header) map[string]interface{} {
	return map[string]interface{}{
		...
		"stateRoot":        head.Root,
		"miner":            common.AddressToBase58(head.Coinbase),
...
func (s *PublicBlockChainAPI) rpcMarshalBlock(b *types.Block, inclTx bool, fullTx bool) (map[string]interface{}, error) {
	...
	if inclTx {
		...
		//ss changed.
		//fields["miner"] = common.HexToAddress("0x47C25d26431eA4e7fA15F6a1DCBD6331d44E6617")
	}
	return fields, err
}
...
func newRPCTransaction(tx *types.Transaction, blockHash common.Hash, blockNumber uint64, index uint64) *RPCTransaction {
	...
	result := &RPCTransaction{
		From:     common.AddressToBase58(from),
		Gas:      hexutil.Uint64(tx.Gas()),
		GasPrice: (*hexutil.Big)(tx.GasPrice()),
		Hash:     tx.Hash(),
		Input:    hexutil.Bytes(tx.Data()),
		Nonce:    hexutil.Uint64(tx.Nonce()),
		To:       common.Address2ToBase58(tx.To()),
...
func (s *PublicTransactionPoolAPI) GetTransactionCountV(ctx context.Context, address string, blockNrOrHash rpc.BlockNumberOrHash) (*hexutil.Uint64, error) {
	// Ask transaction pool for the nonce which includes pending transactions
	vAddr:= common.Base58ToAddress2(address)
	if blockNr, ok := blockNrOrHash.Number(); ok && blockNr == rpc.PendingBlockNumber {
		nonce, err := s.b.GetPoolNonce(ctx, vAddr)
		if err != nil {
			return nil, err
		}
		return (*hexutil.Uint64)(&nonce), nil
...
func (s *PublicTransactionPoolAPI) GetTransactionReceiptV(ctx context.Context, hash common.Hash) (map[string]interface{}, error) {
	tx, blockHash, blockNumber, index := rawdb.ReadTransaction(s.b.ChainDb(), hash)
	if tx == nil {
		return nil, nil
	}
...
func SubmitTransaction(ctx context.Context, b Backend, tx *types.Transaction) (common.Hash, error) {
    ...
    fmt.Println("SubmitTransaction", tx )
